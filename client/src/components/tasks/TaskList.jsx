    ...
    try {
      ...
      const response = await axios.get('http://localhost:8081/api/tasks', {
        ...
        params: {
          ...
          updatedStatus: statusQuery, // Add the updatedStatus parameter
          dueDate: dueDateQuery, // Add the dueDate parameter
        },
      });
      setTasks(response.data.tasks);
    } catch (error) {
...
{/* <input
  ...
  className="p-2 border border-gray-300 rounded-md mr-2"
/> */}
<select className="p-2 border border-gray-300 rounded-md mr-2" placeholder="Search Status" type="text" name="updatedStatus" id="updatedStatus" value={statusQuery} onChange={(e) => setStatusQuery(e.target.value)}>
                    <option value=""></option>
          ...
          {tasks.map((task) => (
            <div key={task._id} className="flex-shrink-0">
              {editableTask && editableTask._id === task._id ? (
                <div>
                  <input type="text" name="aim" value={editableTask.aim} onChange={handleInputChange} />
                  <input type="text" name="assignees" value={editableTask.assignees} onChange={handleInputChange} />
                  <input type="text" name="dueDate" value={editableTask.dueDate} onChange={handleInputChange} />
